        ...
        public AlgorithmPythonWrapper(string moduleName)
        {
            try
            {
                using (Py.GIL())
                {
                    ...
                    // Define the base class of the algorithm, if not found in the module, throw an exception
                    PyObject baseClass = null;
                    if (module.HasAttr("QCAlgorithm"))
                    {
                        baseClass = module.GetAttr("QCAlgorithm");
                    }
                    else
                    {
                        throw new Exception("The module does not contain the QCAlgorithm nor the QCAlgorithmFramework attribute. Please contact support@quantconnect.com.");
                    }
                    ...
                    foreach (var name in module.Dir())
                    {
                        ...
                        if (attr.Repr().Contains(moduleName) && attr.IsSubclass(baseClass))
                        {
